  ...
  const freeShippingPrice = shippingOptions
    .map((shippingOption) => {
      ...
      const validCurrencyPrices = shippingOption.prices.filter(
        (price) =>
          price.currency_code === cart.currency_code && (price.price_rules || []).some((priceRule) => priceRule.attribute === 'item_total'),
      );
    ...
    <div className='bg-neutral-100 p-2 rounded-lg border'>
      <div className='space-y-1.5'>
        ...
        <div className='flex justify-between gap-1'>
          <div
            className={clx('bg-gradient-to-r from-zinc-400 to-zinc-500 h-1 rounded-full max-w-full duration-500 ease-in-out', {
...
function FreeShippingPopup({ cart, price }: { cart: StoreCart; price: StoreFreeShippingPrice }) {
  ...
  return (
    <div
      className={clx('fixed bottom-5 right-5 flex flex-col items-end gap-2 transition-all duration-500 ease-in-out z-10', {
      ...
      <div className='w-[400px] bg-black text-white p-6 rounded-lg '>
        <div className='pb-4'>
          <div className='space-y-3'>
            ...
            <div className='flex justify-between gap-1'>
              <div
                className={clx('bg-gradient-to-r from-zinc-400 to-zinc-500 h-1.5 rounded-full max-w-full duration-500 ease-in-out', {
                  'from-green-400 to-green-500': price.target_reached,
                })}
                style={{ width: `${price.remaining_percentage}%` }}
              ></div>
