...
export const getPricesForVariant = (variant: VariantWithPrice) => {
  ...
  return {
    ...
    percentage_diff: getPercentageDiff(variant.calculated_price.original_amount, variant.calculated_price.calculated_amount),
  };
};
...
export function getProductPrice({ product, variantId }: { product: HttpTypes.StoreProduct; variantId?: string }) {
  ...
  const variantPrice = () => {
    ...
    const variant = product.variants?.find((v) => v.id === variantId || v.sku === variantId) as VariantWithPrice | undefined;
